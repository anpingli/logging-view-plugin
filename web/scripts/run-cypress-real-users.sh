...
export CYPRESS_BASE_URL="https://$(oc get route console -n openshift-console  -o jsonpath={.spec.host} || exit 1)"
export CYPRESS_OPENSHIFT_VERSION=$(oc version -o json|jq  -r '.openshiftVersion' |cut -d. -f1,2 || exit 1)
