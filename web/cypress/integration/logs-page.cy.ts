...
const LABEL_POD_VALUES_URL_MATCH =
  '/api/proxy/plugin/logging-view-plugin/backend/api/logs/v1/application/loki/api/v1/label/kubernetes_container_name/values?*';
...
describe('Logs Page', () => {
  ...
  it('container selection includes loki labels and k8s resources', () => {
    ...
    cy.intercept(RESOURCE_URL_MATCH, podsListResponse).as('resourceQuery');
    cy.intercept(LABEL_POD_VALUES_URL_MATCH, containersLabelValuesResponse).as(
    ...
    cy.getByTestId(TestIds.AttributeFilters).within(() => {
      ...
      cy.contains(/^my-container-from-labels$/).click({ force: true });

      cy.contains(/^my-container-from-labels$/)
        .find('input')
        .should('be.checked');
    ...
    cy.getByTestId(TestIds.LogsQueryInput).within(() => {
      cy.get('textarea')
        ...
        .should(
          'equal',
          '{ kubernetes_container_name=~"my-container-from-labels|operator", kubernetes_pod_name="my-pod-2" } | json',
        );
    });
