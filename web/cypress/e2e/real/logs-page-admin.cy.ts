...
const QEURY_STATEMENT1 = 'kubernetes_namespace_name="log-test-app1" | json';
const QEURY_INFRA_STATEMENT1 = 'kubernetes_namespace_name="openshift-apiserver"|json';
const QEURY_INFRA_STATEMENT2 = 'log_type="infrastructure" |json|systemd_t_COMM="kubelet"';
...
describe('Logs Page', () => {
  ...
  it('query applicatioins logs', () => {
    ...
    cy.getByTestId(TestIds.LogsQueryInput).within(() => {
      cy.get('textarea').type('{selectAll}' + QEURY_STATEMENT1,{ delay: 0 });
    });
  ...
  it('query infra container logs', () => {
    ...
    cy.getByTestId(TestIds.ShowQueryToggle).click();
    cy.getByTestId(TestIds.LogsQueryInput).within(() => {
      cy.get('textarea').clear().type(QEURY_INFRA_STATEMENT1, { delay: 0 });
    });
    cy.getByTestId(TestIds.ExecuteQueryButton).click();
    cy.getByTestId(TestIds.LogsTable).should('exist')
  ...
  it('query infra node logs', () => {
    ...
    cy.getByTestId(TestIds.ShowQueryToggle).click();
    cy.getByTestId(TestIds.LogsQueryInput).within(() => {
      cy.get('textarea').type('{selectAll} ' + QEURY_INFRA_STATEMENT2, { delay: 0 });
    });
    cy.getByTestId(TestIds.ExecuteQueryButton).click();
    cy.getByTestId(TestIds.LogsTable)
      .should('exist')
      .within(() => {
        ...
        cy.get('td.pf-v5-c-table__td.lv-plugin__table__details')
          ...
          .within(() => {
            ...
            cy.contains('span.pf-v5-c-description-list__text', 'openshift_cluster_id');
            cy.contains('span.pf-v5-c-description-list__text', 'openshift_sequence');
            cy.contains('span.pf-v5-c-description-list__text', '_RUNTIME_SCOPE');
  ...
  it('query with the selected content', () => {
    ...
    cy.getByTestId(TestIds.AttributeFilters)
      .should('exist')
      .within(() => {
          cy.get('input[aria-label="Search by Content"]').clear().type(APP_NAMESPACE1, {delay: 200})
      })
    cy.getByTestId(TestIds.ShowQueryToggle).click();
    cy.get('div[data-test="LogsQueryInput"] textarea').should('have.value', '{ log_type="application" } |= `log-test-app1` | json')	
