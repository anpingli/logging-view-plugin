...
declare global {
  namespace Cypress {
    interface Chainable<Subject> {
      ...
      uiLogout();
      uiLoginAsUser(rank: string);
      uiLogoutAsUser(rank: string);
      uiLoginAsClusterAdmin(rank: string);
      uiLogoutClusterAdmin(rank: string);
      grantProjectLogRoles(rank: string, project: string);
      removeProjectLogRoles(rank: string, project: string);
...
Cypress.Commands.add('uiLoginAsUser', (rank: string) => {
  let username="";
  let userpassword="";
  if (Cypress.env('LOGIN_USERS')){
    username=Cypress.env('LOGIN_USERS').split(',')[rank_2_num[rank]].split(':')[0];
...
Cypress.Commands.add('grantProjectLogRoles', (rank: string, project: string) => {
  let username="";
  if (Cypress.env('LOGIN_USERS')){
    username=Cypress.env('LOGIN_USERS').split(',')[rank_2_num[rank]].split(':')[0];
  }
...
Cypress.Commands.add('removeProjectLogRoles', (rank: string, project: string) => {
  let username="";
  if (Cypress.env('LOGIN_USERS')){
    username=Cypress.env('LOGIN_USERS').split(',')[rank_2_num[rank]].split(':')[0];
  }
...
Cypress.Commands.add('uiLogoutAsUser', (rank: string) => {
  cy.log('Logout user');
  let username = "";
  if (Cypress.env('LOGIN_USERS')){
    username=Cypress.env('LOGIN_USERS').split(',')[rank_2_num[rank]].split(':')[0];
